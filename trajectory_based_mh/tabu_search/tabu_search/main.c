...
int main(int argc, char** argv) {
    ...
    if(argc >= 5 && strncmp(argv[1], "-f", strlen("-f")) == 0) {
        ...
        result = tabu_search_file(argv[2]);
    }
        ...
        printf("Command for tabu search: %s -a [n_cities] [distances_file]\n", argv[0]);
        printf("Command for tabu search from file: %s -f [random_file] [n_cities] [distances_file]\n", argv[0]);
        exit(1);
    }
